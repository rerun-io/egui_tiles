...
mod tests {
    ...
    /// application re-creates its [`Tree`] from scratch every frame — see
    /// `examples/tree_recreated_every_frame.rs` for the same setup as a runnable app.
    ///
    /// This holds today. It is worth pinning down because it is easy to break: anything that
    /// edits the tree mid-frame and expects to put it back the way it was has to survive the
    /// tree being thrown away and rebuilt underneath it.
    #[test]
    fn dropping_a_pane_never_loses_it_when_tree_recreated_every_frame() {
        let ctx = egui::Context::default();

        // Two panes side by side — the smallest interesting case.
        ...
        // Press on "a", drag over "b", release:
